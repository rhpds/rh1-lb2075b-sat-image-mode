...
[arabic]
1. Click on the "`Container images`" menu.

.Booted container images
image::container-images.png[]

2. You will be able to see that one image mode host is detected.
...
Next, under the "`Hosts`" column, click on number `1` link.
...
Expand the job output, by clicking on the accordion button next to the server name `rhel2.lab`.
